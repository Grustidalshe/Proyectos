...
int is_surrounded_by_walls(/*t_vars *vars*/ char **map, int x, int y) {
    if ((y - 1 < 0 || map[y - 1][x] == '1') &&
        (y + 1 >= 0 /*vars->map_y_len*/ || map[y + 1][x] == '1') &&
        (x - 1 < 0 || map[y][x - 1] == '1') &&
        (x + 1 >= 0 /*vars->map_x_len*/ || map[y][x + 1] == '1')) {
        return (1);
    }
    return (0);
}
int validate_map(char **map, int rows, int cols) {
    ...
    // Verificar que haya exactamente un jugador y una salida
