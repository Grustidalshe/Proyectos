...
/*   Updated: 2024/08/19 17:33:05 by vtrubina         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	get_value(t_vars *vars, t_check check)
{
	...
	while (vars->map[i])
	{
		...
		while (vars->map[i][j])
		{
			...
			if (vars->map[i][j] == 'P')
			{
				if (vars->map[i+1][j] == '1' && vars->map[i-1][j] == '1' && vars->map[i][j+1] == '1' && vars->map[i][j-1] == '1' \
				 && vars->map[i+1][j-1] == '1' && vars->map[i+1][j+1] == '1' && vars->map[i-1][j+1] == '1' && vars->map[i-1][j-1] == '1')
				{	
					printf("Hola");
					exit(1);
